...
type APIKey struct {
	...
	// Key-level model whitelist (empty = allow all); exact IDs or trailing '*' wildcard
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case apikey.FieldIPWhitelist, apikey.FieldIPBlacklist, apikey.FieldAllowedModels:
			values[i] = new([]byte)
